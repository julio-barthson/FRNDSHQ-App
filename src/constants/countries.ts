/**
 * ISO 3166-1 alpha-2 codes with their E.164 calling codes.
 *
 * `POST /auth/register` validates `country` with `@IsISO31661Alpha2` and
 * `phoneNumber` against `^\+[1-9]\d{6,14}$`, so the picker supplies both: the
 * code the API wants, and the dial prefix that makes the number valid.
 *
 * Packed as one string rather than 200-odd object literals — the list is data,
 * and this way the whole thing stays scannable.
 */
const PACKED = `
AF Afghanistan 93|AL Albania 355|DZ Algeria 213|AD Andorra 376|AO Angola 244
AG Antigua and Barbuda 1268|AR Argentina 54|AM Armenia 374|AU Australia 61
AT Austria 43|AZ Azerbaijan 994|BS Bahamas 1242|BH Bahrain 973|BD Bangladesh 880
BB Barbados 1246|BY Belarus 375|BE Belgium 32|BZ Belize 501|BJ Benin 229
...
CI Côte d'Ivoire 225|HR Croatia 385|CU Cuba 53|CY Cyprus 357|CZ Czechia 420
...
ER Eritrea 291|EE Estonia 372|SZ Eswatini 268|ET Ethiopia 251|FJ Fiji 679
FI Finland 358|FR France 33|GA Gabon 241|GM Gambia 220|GE Georgia 995
DE Germany 49|GH Ghana 233|GR Greece 30|GD Grenada 1473|GT Guatemala 502
...
IR Iran 98|IQ Iraq 964|IE Ireland 353|IL Israel 972|IT Italy 39|JM Jamaica 1876
JP Japan 81|JO Jordan 962|KZ Kazakhstan 7|KE Kenya 254|KI Kiribati 686
KW Kuwait 965|KG Kyrgyzstan 996|LA Laos 856|LV Latvia 371|LB Lebanon 961
LS Lesotho 266|LR Liberia 231|LY Libya 218|LI Liechtenstein 423|LT Lithuania 370
LU Luxembourg 352|MO Macao 853|MG Madagascar 261|MW Malawi 265|MY Malaysia 60
...
NL Netherlands 31|NZ New Zealand 64|NI Nicaragua 505|NE Niger 227
NG Nigeria 234|KP North Korea 850|MK North Macedonia 389|NO Norway 47
OM Oman 968|PK Pakistan 92|PW Palau 680|PS Palestine 970|PA Panama 507
PG Papua New Guinea 675|PY Paraguay 595|PE Peru 51|PH Philippines 63
PL Poland 48|PT Portugal 351|PR Puerto Rico 1787|QA Qatar 974|RO Romania 40
...
KR South Korea 82|SS South Sudan 211|ES Spain 34|LK Sri Lanka 94|SD Sudan 249
...
export interface Country {
  ...
  code: string;
  name: string;
  /** Without the leading `+`, e.g. `234`. */
  dialCode: string;
}

export const COUNTRIES: Country[] = PACKED.split(/[|\n]/)
  .map((entry) => entry.trim())
  .filter(Boolean)
  .map((entry) => {
    // `CI Côte d'Ivoire 225` — the name is everything between the code and the
    // dial code, so it may contain spaces.
    const parts = entry.split(' ');
    return {
      code: parts[0],
      name: parts.slice(1, -1).join(' '),
      dialCode: parts[parts.length - 1],
    };
  })
  .sort((a, b) => a.name.localeCompare(b.name));

/** Nigeria first — the launch market. */
